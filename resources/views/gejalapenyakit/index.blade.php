...
@section('content')
    ...
    <div class="col-sm-12">
        <div class="page-header float-right mt-2">
            <a href="gejalapenyakit/tambah" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Tambah</a>
        </div>
    </div>
                                            ...
                                            <td>
                                                @if ($item->gambar)
                                                    <img style="max-width : 50px; max-height:50px"
                                                        src="{{ url('gambarpenyakit') . '/' . $item->gambar }}" />
                                                @endif
                                            </td>

                                            <td>
                                                <a href="gejalapenyakit/edit/{{ $item->id }}"
                                                    class=" mb-2 fa fa-edit bg-warning p-2 text-white rounded"></a>
                                                <a href="gejalapenyakit/hapus/{{ $item->id }}"
                                                    onclick="return confirm('Yakin Ingin Menghapus Data')"
                                                    class=" mb-2 fa fa-trash bg-danger p-2 text-white rounded"></a>
                                                <a href="{{ route('gejalapenyakit.show', $item->id) }}"
