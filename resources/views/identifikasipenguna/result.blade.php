                            ...
                            <div class="card-body">
                               <tbody>                      
                                    @for ($i = 0; $i < count($penyakit); $i++)
                                        <p class="arial">Tanaman Jagung Anda Terkena Hama atau Penyakit  :  <b>{{$penyakit [$i]}}</b><p>
                                            <p class="lower">Presentasi : <b>{{$hasil [$i]}}</b></p><br>
                                            <p class="lower">berikut gambar dari :  <b>{{$penyakit [$i]}}</b></p><br>
                                                    <img style="max-width : 250px; max-height:300px" 
                                                    src="{{ asset('gambar/' .$gambar[$i])}}" ><br><br>
                                            <p class="lower">Solusi Dari <b>{{$penyakit [$i]}}</b> adalah sebagai berikut:</p><br>
                                            <p class="lower">{{$solosi [$i]}}</p><br><br><br>
                                        @endfor
                                            <br>
                                            <br>
                                     <a href='/identifikasipenguna' class="btn btn-secondary"><< Kembali</a>
                                </tbody>
                            </div>
                        </div>
                    </div>
